...
background-image-resolution: from-image;">
    ...
    <div class="dados">
        <p id="p1">{{ $aluno->vc_primeiroNome . ' ' . $aluno->vc_nomedoMeio . ' ' . $aluno->vc_ultimoaNome }},
            aluno (a)
            matriculado nesta Instituição sob o Processo nº {{ $aluno->id }}, na</p>
        <p id="p2">Turma {{ $matricula->vc_nomedaTurma }} Sala {{ $matricula->vc_salaTurma }} da
            {{ $matricula->vc_classeTurma }}ª Classe, com o número de Ordem {{ $n_ordem }} do Curso de </p>
        <p id="p3"><span style="text-transform: capitalize;">{{ $matricula->vc_nomeCurso }}</span> Tendo faltado
            na
            Aula/Prova de </p>
        <p id="p4">
    ...
    <div class="data">
        <span>Data: {{ date('d') }} de <span
                style="text-transform: capitalize;">{{ strftime('%B', date('m')) }}</span> de
            {{ date('Y') }}</span>
    </div>
