...
background-image-resolution: from-image;">
    ...
    <div class="dados">
        <p id="p1">Para efeitos de {{ $efeito }} declara-se que
            {{ $aluno->vc_primeiroNome . ' ' . $aluno->vc_nomedoMeio . ' ' . $aluno->vc_ultimoaNome }}, filho de
            {{ $aluno->vc_namePai }} e de {{ $aluno->vc_nameMae }}, natural de {{ $aluno->vc_naturalidade }}, Província
            de {{ $aluno->vc_provincia }}, nascido aos {{ date('d', strtotime($aluno->dt_dataNascimento)) }} de <span
                style="text-transform: capitalize;">{{ strftime('%B', date('m', strtotime($aluno->dt_dataNascimento))) }}</span>
            de {{ date('Y', strtotime($aluno->dt_dataNascimento)) }} &nbsp;&nbsp; portador &nbsp; do
            &nbsp; Bilhete &nbsp; de &nbsp;Identidade nº {{ $aluno->vc_bi }}, Passado pelo Arquivo de &nbsp;
            Identificação &nbsp; de &nbsp; {{ $aluno->vc_localEmissao }}, aos
            {{ date('d', strtotime($aluno->dt_emissao)) }} de
            <span
                style="text-transform: capitalize;">{{ strftime('%B', date('m', strtotime($aluno->dt_emissao))) }}</span>
            de {{ date('Y', strtotime($aluno->dt_emissao)) }}, matriculado sob nº {{ $aluno->id }}, frequenta no
            ano lectivo {{ $ano_lectivo->ya_inicio }}/{{ $ano_lectivo->ya_fim }} o curso
            <span style="text-transform: capitalize;">{{ $matricula->vc_nomeCurso }}</span> da Área &nbsp; de &nbsp; formação &nbsp; de______________________, da
            formação profissional média em regime <b>Diurno/Nocturno</b>, na turma
            &nbsp;{{ $matricula->vc_nomedaTurma }} &nbsp; com o nº{{ $n_ordem }},
        </p>
