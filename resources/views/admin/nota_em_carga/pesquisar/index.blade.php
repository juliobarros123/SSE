...
@section('conteudo')
    ...
    {{-- <div class="card">
        <div class="card-body">
            <form action="{{ route('nota_em_carga.ver') }}" class="row" method="POST">
                @csrf
    ...
    <div class="card">
        <div class="card-body">
            <form method="POST" class="row" action="{{ route('nota_em_carga.ver') }}">
                @csrf
                <div class="col-sm-4">
                        ...
                        <select name="id_turma_user" class="form-control select-dinamico" required>
                            @foreach ($turmasUser as $turmaUser)
                                <option value="{{ $turmaUser->id_turma_user }}">
                                    ...
                                    ªClasse||{{ $turmaUser->vc_nomedaTurma }}||{{ $turmaUser->vc_nome }}|| {{ $turmaUser->vc_anoLectivo }}</option>
                            @endforeach
                        </select>
